        ...
        if (Input.touchCount > 0)
        {
            if (Input.GetTouch(0).phase == TouchPhase.Ended)
            {
                ...
                if (_step < _currentBranch.Count && DialogController._dialogController.MightSetDialog == true)
                {
                    ...
                    DialogController._dialogController.SetDialog(_currentBranch[_step].Speaker.Name, _currentBranch[_step].Speaker.NameColor, _currentBranch[_step].PhraseText);
                    
                    _step++;

                    if (_step >= _currentBranch.Count)
                    {
                        _currentBranch[_step].TriggeringAfterActions();
                    }
                }
            }
        }
